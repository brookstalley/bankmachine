...
| norm lives in `.prawduct/artifacts/api-contract.md` § Direction — a stored balance is reported with its lifecycle, and no total over balances is emitted without stating its treatment of non-active accounts (treatment ruled: include and flag) | Test | — (FR-9 lands it; the guard must assert the FIGURE and not only the flag, and be seen red with the magnitude removed) | janitor | Why lives in the Direction entry. Recorded `in-transition`, deliberately: the norm binds now and its mechanism arrives with FR-9, and the one shipped emitter it governs (`query._coverage`'s unfiltered `accounts` count) does not yet carry the figure. Claiming `Test` against a mechanism that does not exist is this table's named aspirational failure — the row flips to a real enforcement artifact when AC-12.8's guard exists and has gone red. 🔴 The guard checks the magnitude, because a flag alone passes the norm's letter and fails the reason it was born: a consumer told something is wrong with no figure to act on. |
