...
| norm lives in `.prawduct/artifacts/architecture.md` § Direction — one writer process at a time, serialized on a lockfile before any write transaction | Test | — | janitor | Why lives in the Direction entry. Mechanism filed as ARC-7K2M until the scaffold lands. |
| norm lives in `.prawduct/artifacts/architecture.md` § Direction — the MCP server opens `query_only` and holds no read transaction across tool calls | Test | — | janitor | Why lives in the Direction entry. Mechanism filed as ARC-7K2M until the scaffold lands. |
| norm lives in `.prawduct/artifacts/architecture.md` § Direction — no component creates the datastore implicitly | Test | — | janitor | Why lives in the Direction entry. Mechanism filed as ARC-7K2M until the scaffold lands. |
| norm lives in `.prawduct/artifacts/architecture.md` § Direction — a process that does not recognize the schema version refuses to serve | Test | — | janitor | Why lives in the Direction entry. Mechanism filed as ARC-7K2M until the scaffold lands. |
