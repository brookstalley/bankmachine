...
| norm lives in `.prawduct/artifacts/architecture.md` § Direction — every writable handle comes from the one writer factory, which takes the lock before it returns | Test | — | janitor | Why lives in the Direction entry. Mechanism filed as issue #1 until the scaffold lands. |
| norm lives in `.prawduct/artifacts/architecture.md` § Direction — read-role handles open `mode=ro`, hold no cross-call read transaction, and never fall back to a writable handle | Test | — | janitor | Why lives in the Direction entry. Mechanism filed as issue #1 until the scaffold lands. |
| norm lives in `.prawduct/artifacts/architecture.md` § Direction — no component creates the datastore implicitly | Test | — | janitor | Why lives in the Direction entry. Mechanism filed as issue #1 until the scaffold lands. |
| norm lives in `.prawduct/artifacts/architecture.md` § Direction — a process that does not recognize the schema version refuses to serve | Test | — | janitor | Why lives in the Direction entry. Mechanism filed as issue #1 until the scaffold lands. |
