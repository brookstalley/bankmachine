...
| norm lives in `.prawduct/artifacts/architecture.md` § Direction — a process that does not recognize the schema version refuses to serve | Test | `tests/store/test_connection_norms.py` — reader refusal, **writer refusal** (both roles call one `_require_supported_schema`; only `initializing_writer` skips it, because opening an old version is the migration runner's job), `store status` reporting, and migration atomicity under a real kill | janitor | Why lives in the Direction entry. Mechanism landed in Chunk 01, closing issue #1; each test was verified to go red with its norm deliberately broken (`tests/preferences/verify_norms_go_red.py`). |

| norm lives in `.prawduct/artifacts/data-model.md` § Direction — every stored amount is signed from the operator's point of view; a card balance is stored negative | Critic | — (pending `brookstalley/bankmachine#9`, the connector normalization test) | janitor | Why lives in the Direction entry. **`Status: in-transition`** — ratified 2026-09-07 with no mechanism yet, deliberately: the code that must obey it is mid-build, and asserting enforcement that does not exist is the aspirational failure. Flips to steady-state when #9 is green and has been seen red. |
...
| norm lives in `.prawduct/artifacts/data-model.md` § Direction — the daily balance and holdings series are append-only | Test | Composite primary keys on `balances_daily` and `holdings` in the DDL, compared by `tests/store/test_schema.py` | janitor | Why lives in the Direction entry. The one class of data no re-sync can reconstruct. |
...
| norm lives in `.prawduct/artifacts/api-contract.md` § Direction — the MCP surface is read-only; there are no mutation tools | Critic | Partly structural today via `mode=ro` (`tests/store/test_connection_norms.py`); the tool surface itself is build step 7 | janitor | Why lives in the Direction entry. Born before the surface exists, on this repo's own precedent — the point is that step 7 is built to it rather than discovering it. |
...
| norm lives in `.prawduct/artifacts/operational-spec.md` § Direction — nothing creates a datastore, or a backup, implicitly | Test | `tests/store/test_connection_norms.py` (missing-datastore error, non-creating `mode=rw`), `tests/store/test_backup.py` (refuses an existing destination, refuses an absent directory, leaves no file on failure) | janitor | Why lives in the Direction entry. A typo'd path must be reported, not populated. |
