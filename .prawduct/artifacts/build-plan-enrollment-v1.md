...
governed_by:
  ...
  - artifact: data-model
    dispositions:
      ...
      - "every silver row is either aggregator-sourced or manually imported, carries the evidence for which, and carries the derivation version that produced it → inapplicable to what this plan writes, and the reason matters. `institutions` and `connections` carry neither `derivation_version_id` nor a foreign key pointing at a raw response, which is what `rebuild.py` uses to classify a table: they are enrollment rows, the identity/operator state a rebuild explicitly never touches, because `credential_ref` appears in no response and nothing in the archive could recreate it. The norm governs the derived silver tables — accounts, transactions, balances_daily, holdings — none of which this plan writes. 🔴 An earlier draft of this line claimed `connections` carries a `raw_response_id`; it does not, and the claim was corrected rather than the schema being changed to match it"
